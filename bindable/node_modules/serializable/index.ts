...
if (!console['Console']) {
    ...
    let nodeRequireModule = require;
    let fs: typeof NodeFS = nodeRequireModule('fs'); //
    let vm: typeof NodeVM = nodeRequireModule('vm');//
    DynamicRequire = (fileName: string, directoryName: string, callback: (exported: any) => void) => {
        try {
            if (!directoryName) directoryName = __relativeRoot;
            //console.log('DynamicRequire: ', fileName, ' Base Path: ' + directoryName);
...
class ObjectDeserializer {
    constructor(private jsonObject: any, private callback: (deserialized: any) => void) {
        //console.log('Object for Deserialization: ', jsonObject);
        ...
        if (typeof jsonObject == 'object') this.result = {};
    ...
    private DeserializeProperties = () => {
        for (let property in this.jsonObject) {
            ...
            //console.log('Deserialize property: ', property, this.properties);
    ...
    private onPropertyDeserialized = (property: string) => {
        return (deserialized: any) => {
            ...
            //console.log('prorperty deserialized: ', property, deserialized, this.result);
    ...
    private invokeCallback = () => {
        //console.log('try invoke callback.', this.indices.length, this.properties.length, this.started);
        if (this.indices.length == 0 && this.properties.length == 0 && this.started) {
            //console.log('Deserialization result: ', this.result);
